...
1. Starts BGM if gameplay is active and music has not begun yet.
...
5. If a transition is active:
   - draw frozen copies of all active world arrays
   ...
   - return early without advancing gameplay state
...
`collisionCooldown` suppresses repeated rapid stress hits from asteroid and enemy bullet collisions.
